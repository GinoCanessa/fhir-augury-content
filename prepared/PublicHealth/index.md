# Public Health — Prepared Tickets

This index lists all **44** prepared tickets for the **Public Health** work group, grouped by specification.

> **Prepared tickets** have been reviewed and have proposed dispositions ready for work-group consideration. They are sorted by date within each group so the oldest outstanding items are addressed first.

---

## Table of Contents

- [Electronic Case Reporting (FHIR)](#electronic-case-reporting-fhir)
- [Electronic Case Reporting [deprecated] (FHIR)](#electronic-case-reporting-deprecated-fhir)
- [National Healthcare Safety Network (NHSN) Digital Quality Measure (dQM) Reporting Implementation Guide (FHIR)](#national-healthcare-safety-network-nhsn-digital-quality-measure-dqm-reporting-implementation-guide-fhir)
- [Situation Awareness for Novel Epidemic Response (FHIR)](#situation-awareness-for-novel-epidemic-response-fhir)
- [Situation Awareness for Novel Epidemic Response (FHIR) (SANER)](#situation-awareness-for-novel-epidemic-response-fhir-saner)
- [Situation Awareness for Novel Epidemic Response (SANER) (FHIR)](#situation-awareness-for-novel-epidemic-response-saner-fhir)
- [US Bidirectional Services eReferrals (BSeR) (FHIR)](#us-bidirectional-services-ereferrals-bser-fhir)
- [US Central Cancer Registry Reporting Content IG (FHIR)](#us-central-cancer-registry-reporting-content-ig-fhir)
- [US Healthcare Associated Infection Reports Long Term Care Facilities (HAI-LTCF) (FHIR)](#us-healthcare-associated-infection-reports-long-term-care-facilities-hai-ltcf-fhir)
- [US Healthcare Associated Infections (FHIR)](#us-healthcare-associated-infections-fhir)
- [US Helios Query and Response (FHIR)](#us-helios-query-and-response-fhir)
- [US Occupational Data for Health (FHIR)](#us-occupational-data-for-health-fhir)
- [US Public Health Profiles Library (FHIR)](#us-public-health-profiles-library-fhir)
- [US Vital Records Death Reporting (VRDR) (FHIR)](#us-vital-records-death-reporting-vrdr-fhir)
- [Unspecified](#unspecified)
- [Vital Records Common Profiles Library (FHIR)](#vital-records-common-profiles-library-fhir)

---

## Electronic Case Reporting (FHIR)

*1 ticket(s) targeting the **Electronic Case Reporting (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-54303 | Update guidance on what SHOULD means in the IG (if needed) | Triaged | 2026-01-15 | [FHIR-54303](./FHIR-54303.md) · [Jira](https://jira.hl7.org/browse/FHIR-54303) |

---

## Electronic Case Reporting [deprecated] (FHIR)

*5 ticket(s) targeting the **Electronic Case Reporting [deprecated] (FHIR)** specification.*

#### Related Group: FHIR-15173

> FHIR-15173 and FHIR-15265 are related and should be worked together. They reference each other in their prerequisites or related-tickets sections — complete them in the listed order.

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-15173 | Index - No summary was provided by the commenter. - 2018-Jan eCR #4 | Triaged | 2018-02-08 | [FHIR-15173](./FHIR-15173.md) · [Jira](https://jira.hl7.org/browse/FHIR-15173) |
| FHIR-15265 | Location Clarification — eCR IG | Triaged | 2018-02-08 | [FHIR-15265](./FHIR-15265.md) · [Jira](https://jira.hl7.org/browse/FHIR-15265) |

#### Related Group: FHIR-15196

> FHIR-15196 and FHIR-15326 are related and should be worked together. They reference each other in their prerequisites or related-tickets sections — complete them in the listed order.

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-15196 | Profiles - No summary was provided by the commenter. - 2018-Jan eCR #18 | Triaged | 2018-02-08 | [FHIR-15196](./FHIR-15196.md) · [Jira](https://jira.hl7.org/browse/FHIR-15196) |
| FHIR-15326 | Update re TEFCA | Triaged | 2018-08-08 | [FHIR-15326](./FHIR-15326.md) · [Jira](https://jira.hl7.org/browse/FHIR-15326) |

#### Other Tickets

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-15212 | Ticket Preparation Report | Triaged | 2018-02-08 | [FHIR-15212](./FHIR-15212.md) · [Jira](https://jira.hl7.org/browse/FHIR-15212) |

---

## National Healthcare Safety Network (NHSN) Digital Quality Measure (dQM) Reporting Implementation Guide (FHIR)

*1 ticket(s) targeting the **National Healthcare Safety Network (NHSN) Digital Quality Measure (dQM) Reporting Implementation Guide (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-47441 | Enable the transmission of smaller packages and for transport mechanisms such as Bulk Data | Triaged | 2024-09-12 | [FHIR-47441](./FHIR-47441.md) · [Jira](https://jira.hl7.org/browse/FHIR-47441) |

---

## Situation Awareness for Novel Epidemic Response (FHIR)

*1 ticket(s) targeting the **Situation Awareness for Novel Epidemic Response (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-29874 | Defining Use Cases to Demonstrate Hospital PPE Data | Triaged | 2020-12-03 | [FHIR-29874](./FHIR-29874.md) · [Jira](https://jira.hl7.org/browse/FHIR-29874) |

---

## Situation Awareness for Novel Epidemic Response (FHIR) (SANER)

*3 ticket(s) targeting the **Situation Awareness for Novel Epidemic Response (FHIR) (SANER)** specification.*

#### Related Group: FHIR-29848

> FHIR-29848 and FHIR-29854 are related and should be worked together. They reference each other in their prerequisites or related-tickets sections — complete them in the listed order.

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-29848 | Actor requirements listed in Transaction requirements | Triaged | 2020-12-03 | [FHIR-29848](./FHIR-29848.md) · [Jira](https://jira.hl7.org/browse/FHIR-29848) |
| FHIR-29854 | Actor requirements listed in Transaction requirements section | Triaged | 2020-12-03 | [FHIR-29854](./FHIR-29854.md) · [Jira](https://jira.hl7.org/browse/FHIR-29854) |

#### Other Tickets

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-29446 | Consider incorporating concept map from HAVBED to CDC Vocabulary for Bed Location | Triaged | 2020-10-26 | [FHIR-29446](./FHIR-29446.md) · [Jira](https://jira.hl7.org/browse/FHIR-29446) |

---

## Situation Awareness for Novel Epidemic Response (SANER) (FHIR)

*1 ticket(s) targeting the **Situation Awareness for Novel Epidemic Response (SANER) (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-29867 | Proposal for a PPE Inventory Data Standard | Triaged | 2020-12-03 | [FHIR-29867](./FHIR-29867.md) · [Jira](https://jira.hl7.org/browse/FHIR-29867) |

---

## US Bidirectional Services eReferrals (BSeR) (FHIR)

*15 ticket(s) targeting the **US Bidirectional Services eReferrals (BSeR) (FHIR)** specification.*

#### Related Group: FHIR-18206

> FHIR-18206 and FHIR-18248 are related and should be worked together. They reference each other in their prerequisites or related-tickets sections — complete them in the listed order.

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-18206 | Relationship between Observations and DAM | Triaged | 2018-09-24 | [FHIR-18206](./FHIR-18206.md) · [Jira](https://jira.hl7.org/browse/FHIR-18206) |
| FHIR-18248 | Unclear how to send tobacco cessation unique observations | Triaged | 2018-09-24 | [FHIR-18248](./FHIR-18248.md) · [Jira](https://jira.hl7.org/browse/FHIR-18248) |

#### Related Group: FHIR-18252

> FHIR-18252 and FHIR-42220 are related and should be worked together. They reference each other in their prerequisites or related-tickets sections — complete them in the listed order.

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-18252 | Only 1 allergy seems to be permitted | Triaged | 2018-09-24 | [FHIR-18252](./FHIR-18252.md) · [Jira](https://jira.hl7.org/browse/FHIR-18252) |
| FHIR-42220 | Relax cardinality on Task.for | Triaged | 2023-08-31 | [FHIR-42220](./FHIR-42220.md) · [Jira](https://jira.hl7.org/browse/FHIR-42220) |

#### Related Group: FHIR-18254

> FHIR-18254 and FHIR-18255 are related and should be worked together. They reference each other in their prerequisites or related-tickets sections — complete them in the listed order.

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-18254 | Unclear why their is class diagram here | Triaged | 2018-09-24 | [FHIR-18254](./FHIR-18254.md) · [Jira](https://jira.hl7.org/browse/FHIR-18254) |
| FHIR-18255 | Guidance on how to USE the artifacts | Triaged | 2018-09-24 | [FHIR-18255](./FHIR-18255.md) · [Jira](https://jira.hl7.org/browse/FHIR-18255) |

#### Other Tickets

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-18205 | Arthritis Observation has code inconsistency | Triaged | 2018-09-24 | [FHIR-18205](./FHIR-18205.md) · [Jira](https://jira.hl7.org/browse/FHIR-18205) |
| FHIR-18214 | Observation.status and Observation.code are inconsistent between observation profiles | Triaged | 2018-09-24 | [FHIR-18214](./FHIR-18214.md) · [Jira](https://jira.hl7.org/browse/FHIR-18214) |
| FHIR-18215 | Should Body Height valueQuantity be constrained to centimeters? | Triaged | 2018-09-24 | [FHIR-18215](./FHIR-18215.md) · [Jira](https://jira.hl7.org/browse/FHIR-18215) |
| FHIR-18250 | Broken Links to Common Transaction Profiles | Triaged | **Type:** Change Request | **Priority:** Medium | 2018-09-24 | **Updated:** 2019-02-23 | [FHIR-18250](./FHIR-18250.md) · [Jira](https://jira.hl7.org/browse/FHIR-18250) |
| FHIR-21649 | Consider profiling ServiceRequest with slices for supporting information | Triaged | 2019-05-07 | [FHIR-21649](./FHIR-21649.md) · [Jira](https://jira.hl7.org/browse/FHIR-21649) |
| FHIR-22602 | Include brief intro section to all profiles? (Similar to that for BSeR Arthritis Feedback Observation) - BSeR #138 | Triaged | 2019-08-29 | [FHIR-22602](./FHIR-22602.md) · [Jira](https://jira.hl7.org/browse/FHIR-22602) |
| FHIR-42519 | Why does Task not have standard codes for inputs and outputs? | Triaged | 2023-09-02 | [FHIR-42519](./FHIR-42519.md) · [Jira](https://jira.hl7.org/browse/FHIR-42519) |
| FHIR-42533 | CBO Hub or Healthcare Information Exchange (HIE) Options | Triaged | 2023-09-02 | [FHIR-42533](./FHIR-42533.md) · [Jira](https://jira.hl7.org/browse/FHIR-42533) |
| FHIR-42534 | CBOs and Subscriptions | Triaged | 2023-09-02 | [FHIR-42534](./FHIR-42534.md) · [Jira](https://jira.hl7.org/browse/FHIR-42534) |

---

## US Central Cancer Registry Reporting Content IG (FHIR)

*1 ticket(s) targeting the **US Central Cancer Registry Reporting Content IG (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-35768 | Might want to collaborate and note any overlap or reference with CODEX work, V2 | Triaged | 2022-01-11 | [FHIR-35768](./FHIR-35768.md) · [Jira](https://jira.hl7.org/browse/FHIR-35768) |

---

## US Healthcare Associated Infection Reports Long Term Care Facilities (HAI-LTCF) (FHIR)

*1 ticket(s) targeting the **US Healthcare Associated Infection Reports Long Term Care Facilities (HAI-LTCF) (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-24487 | Can we get rid of the "definition" column if there are no definitions? Tooling. | Triaged | 2019-09-16 | [FHIR-24487](./FHIR-24487.md) · [Jira](https://jira.hl7.org/browse/FHIR-24487) |

---

## US Healthcare Associated Infections (FHIR)

*3 ticket(s) targeting the **US Healthcare Associated Infections (FHIR)** specification.*

#### Related Group: FHIR-23726

> FHIR-23726 and FHIR-23727 are related and should be worked together. They reference each other in their prerequisites or related-tickets sections — complete them in the listed order.

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-23726 | Are the HAI-LTCF value sets used? | Triaged | 2019-08-20 | [FHIR-23726](./FHIR-23726.md) · [Jira](https://jira.hl7.org/browse/FHIR-23726) |
| FHIR-23727 | Not sure what the "other resources" are in HAI-LTCF | Triaged | 2019-08-20 | [FHIR-23727](./FHIR-23727.md) · [Jira](https://jira.hl7.org/browse/FHIR-23727) |

#### Other Tickets

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-21647 | Consider updating from FHIR R3 to FHIR R4 | Triaged | 2019-05-07 | [FHIR-21647](./FHIR-21647.md) · [Jira](https://jira.hl7.org/browse/FHIR-21647) |

---

## US Helios Query and Response (FHIR)

*1 ticket(s) targeting the **US Helios Query and Response (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-51983 | Alignment of minimum necessary versus post-filtering | Triaged | 2026-03-11 | [FHIR-51983](./FHIR-51983.md) · [Jira](https://jira.hl7.org/browse/FHIR-51983) |

---

## US Occupational Data for Health (FHIR)

*3 ticket(s) targeting the **US Occupational Data for Health (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-19146 | US versus International Perspective - ODH #18 | Triaged | 2018-10-02 | [FHIR-19146](./FHIR-19146.md) · [Jira](https://jira.hl7.org/browse/FHIR-19146) |
| FHIR-19148 | Retirement - Future Date Consideration - ODH #19 | Triaged | 2018-10-02 | [FHIR-19148](./FHIR-19148.md) · [Jira](https://jira.hl7.org/browse/FHIR-19148) |
| FHIR-28533 | Reuse "Employment History Episode" from Military Service History IG | Triaged | 2020-09-14 | [FHIR-28533](./FHIR-28533.md) · [Jira](https://jira.hl7.org/browse/FHIR-28533) |

---

## US Public Health Profiles Library (FHIR)

*3 ticket(s) targeting the **US Public Health Profiles Library (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-49700 | Consider using the cqf-expansionParameters extension | Triaged | 2025-01-29 | [FHIR-49700](./FHIR-49700.md) · [Jira](https://jira.hl7.org/browse/FHIR-49700) |
| FHIR-51950 | US PH Characteristics of Home Environment modeling | Triaged | 2025-08-26 | [FHIR-51950](./FHIR-51950.md) · [Jira](https://jira.hl7.org/browse/FHIR-51950) |
| FHIR-52762 | Consider using CRMI MessageDefinition profiles | Triaged | 2025-09-08 | [FHIR-52762](./FHIR-52762.md) · [Jira](https://jira.hl7.org/browse/FHIR-52762) |

---

## US Vital Records Death Reporting (VRDR) (FHIR)

*2 ticket(s) targeting the **US Vital Records Death Reporting (VRDR) (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-38623 | Codes for Part1 and 2 Cause of Death -- Suggestions | Triaged | 2022-09-13 | [FHIR-38623](./FHIR-38623.md) · [Jira](https://jira.hl7.org/browse/FHIR-38623) |
| FHIR-51416 | Add Use Cases related to an EHR-EDRS workflow | Triaged | 2025-07-02 | [FHIR-51416](./FHIR-51416.md) · [Jira](https://jira.hl7.org/browse/FHIR-51416) |

---

## Unspecified

*1 ticket(s) targeting the **Unspecified** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-18251 | Should BSeR Patient profile be based on the US Core profile? | Triaged | 2018-09-24 | [FHIR-18251](./FHIR-18251.md) · [Jira](https://jira.hl7.org/browse/FHIR-18251) |

---

## Vital Records Common Profiles Library (FHIR)

*2 ticket(s) targeting the **Vital Records Common Profiles Library (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-44441 | Code Systems and Valuesets should be anchored in THO (terminology.hl7.org). 21 | Triaged | 2024-01-22 | [FHIR-44441](./FHIR-44441.md) · [Jira](https://jira.hl7.org/browse/FHIR-44441) |
| FHIR-49764 | Missing State/Province/Territory Code to Represent Full IJE Semantics | Triaged | 2025-02-11 | [FHIR-49764](./FHIR-49764.md) · [Jira](https://jira.hl7.org/browse/FHIR-49764) |

---

*Generated automatically. Re-run `generate-indexes.py` to update.*
