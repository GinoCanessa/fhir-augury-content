# Patient Administration — Prepared Tickets

This index lists all **30** prepared tickets for the **Patient Administration** work group, grouped by specification.

> **Prepared tickets** have been reviewed and have proposed dispositions ready for work-group consideration. They are sorted by date within each group so the oldest outstanding items are addressed first.

---

## Table of Contents

- [FHIR Core (FHIR)](#fhir-core-fhir)
- [Interoperable Digital Identity and Patient Matching (FHIR)](#interoperable-digital-identity-and-patient-matching-fhir)
- [National Directory of Healthcare Providers and Services (NDH) (FHIR)](#national-directory-of-healthcare-providers-and-services-ndh-fhir)
- [Real Time Location Services (FHIR)](#real-time-location-services-fhir)
- [US National Directory (FHIR)](#us-national-directory-fhir)
- [Unspecified](#unspecified)
- [Validated Healthcare Directory (FHIR)](#validated-healthcare-directory-fhir)
- [Validated Healthcare Directory (VhDir) (FHIR)](#validated-healthcare-directory-vhdir-fhir)

---

## FHIR Core (FHIR)

*2 ticket(s) targeting the **FHIR Core (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-43009 | Clarify ChargeItemDefinition.factor | Triaged | 2023-10-26 | [FHIR-43009](./FHIR-43009.md) · [Jira](https://jira.hl7.org/browse/FHIR-43009) |
| FHIR-37648 | Better approach for large lists of participants in Encounter.participants | Triaged | 2024-01-30 | [FHIR-37648](./FHIR-37648.md) · [Jira](https://jira.hl7.org/browse/FHIR-37648) |

---

## Interoperable Digital Identity and Patient Matching (FHIR)

*6 ticket(s) targeting the **Interoperable Digital Identity and Patient Matching (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-39629 | Matching Attributes need to relate to search time frame | Triaged | 2022-12-15 | [FHIR-39629](./FHIR-39629.md) · [Jira](https://jira.hl7.org/browse/FHIR-39629) |
| FHIR-40302 | Additional identity text for V2 | Triaged | 2023-01-20 | [FHIR-40302](./FHIR-40302.md) · [Jira](https://jira.hl7.org/browse/FHIR-40302) |
| FHIR-40819 | Ticket Prep for Workgroup Review | Triaged | 2023-04-17 | [FHIR-40819](./FHIR-40819.md) · [Jira](https://jira.hl7.org/browse/FHIR-40819) |
| FHIR-43284 | Clarify when to include B2B with User Authorization Extension Object | Triaged | 2023-11-28 | [FHIR-43284](./FHIR-43284.md) · [Jira](https://jira.hl7.org/browse/FHIR-43284) |
| FHIR-44594 | Add Conformance Tab | Triaged | 2024-01-25 | [FHIR-44594](./FHIR-44594.md) · [Jira](https://jira.hl7.org/browse/FHIR-44594) |
| FHIR-47704 | Create an example of how Digital Identifier appears in a SMART Health Card | Triaged | 2024-09-13 | [FHIR-47704](./FHIR-47704.md) · [Jira](https://jira.hl7.org/browse/FHIR-47704) |

---

## National Directory of Healthcare Providers and Services (NDH) (FHIR)

*3 ticket(s) targeting the **National Directory of Healthcare Providers and Services (NDH) (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-42638 | Elements of this specification should be defined in the core-extension set | Triaged | 2023-09-04 | [FHIR-42638](./FHIR-42638.md) · [Jira](https://jira.hl7.org/browse/FHIR-42638) |
| FHIR-42639 | IHE actor and transaction indication | Triaged | 2023-09-04 | [FHIR-42639](./FHIR-42639.md) · [Jira](https://jira.hl7.org/browse/FHIR-42639) |
| FHIR-55990 | How to show practitioner verification status in NDH | Triaged | 2026-03-02 | [FHIR-55990](./FHIR-55990.md) · [Jira](https://jira.hl7.org/browse/FHIR-55990) |

---

## Real Time Location Services (FHIR)

*10 ticket(s) targeting the **Real Time Location Services (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-40879 | Missing details related to the transactions/interactions | Triaged | 2023-04-21 | [FHIR-40879](./FHIR-40879.md) · [Jira](https://jira.hl7.org/browse/FHIR-40879) |
| FHIR-40965 | Extend the profiles and examples to cover the scope of the specification | Triaged | 2023-04-26 | [FHIR-40965](./FHIR-40965.md) · [Jira](https://jira.hl7.org/browse/FHIR-40965) |
| FHIR-40968 | Suggest using udiCarrier in the RTLS Tag Device Profile | Triaged | 2023-04-26 | [FHIR-40968](./FHIR-40968.md) · [Jira](https://jira.hl7.org/browse/FHIR-40968) |
| FHIR-40969 | Add udiCarrier in the RTLS Device example | Triaged | 2023-04-26 | [FHIR-40969](./FHIR-40969.md) · [Jira](https://jira.hl7.org/browse/FHIR-40969) |
| FHIR-40970 | Add udiCarrier in the Location Update Event example | Triaged | 2023-04-26 | [FHIR-40970](./FHIR-40970.md) · [Jira](https://jira.hl7.org/browse/FHIR-40970) |
| FHIR-41063 | Should the official URL be anchored in terminology.hl7.org, not hl7.org/fhir? 1 | Triaged | 2023-05-01 | [FHIR-41063](./FHIR-41063.md) · [Jira](https://jira.hl7.org/browse/FHIR-41063) |
| FHIR-41064 | Should the official URL be anchored in terminology.hl7.org, not hl7.org/fhir? 2 | Triaged | 2023-05-01 | [FHIR-41064](./FHIR-41064.md) · [Jira](https://jira.hl7.org/browse/FHIR-41064) |
| FHIR-41065 | Should the official URL be anchored in terminology.hl7.org, not hl7.org/fhir? 3 | Triaged | 2023-05-01 | [FHIR-41065](./FHIR-41065.md) · [Jira](https://jira.hl7.org/browse/FHIR-41065) |
| FHIR-41066 | Should the official URL be anchored in terminology.hl7.org, not hl7.org/fhir? 4 | Triaged | 2023-05-01 | [FHIR-41066](./FHIR-41066.md) · [Jira](https://jira.hl7.org/browse/FHIR-41066) |
| FHIR-41167 | No need to use FHIR Messaging when simpler interactions serve the same purpose | Triaged | 2023-05-02 | [FHIR-41167](./FHIR-41167.md) · [Jira](https://jira.hl7.org/browse/FHIR-41167) |

---

## US National Directory (FHIR)

*1 ticket(s) targeting the **US National Directory (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-41604 | extension.restrictFhirPath should be deprecated and reference DS4P extension for partial inline labeling | Triaged | 2023-07-19 | [FHIR-41604](./FHIR-41604.md) · [Jira](https://jira.hl7.org/browse/FHIR-41604) |

---

## Unspecified

*2 ticket(s) targeting the **Unspecified** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-18809 | make example narrative friendlier - VHD #17 | Triaged **Priority:** Medium **Type:** Comment | 2018-09-28 **Updated:** 2025-02-18 **Labels:** None | [FHIR-18809](./FHIR-18809.md) · [Jira](https://jira.hl7.org/browse/FHIR-18809) |
| FHIR-38802 | Split out the concepts of patient and user | — | — | [FHIR-38802](./FHIR-38802.md) · [Jira](https://jira.hl7.org/browse/FHIR-38802) |

---

## Validated Healthcare Directory (FHIR)

*5 ticket(s) targeting the **Validated Healthcare Directory (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-19742 | Change cardinality of Location and PractitionerRole NewPatients extension to (0..*) in SD | Triaged | 2018-12-10 | [FHIR-19742](./FHIR-19742.md) · [Jira](https://jira.hl7.org/browse/FHIR-19742) |
| FHIR-19866 | Update Restriction Examples | Triaged | 2019-01-04 | [FHIR-19866](./FHIR-19866.md) · [Jira](https://jira.hl7.org/browse/FHIR-19866) |
| FHIR-20674 | Remove encounter element from VHDir CareTeam profile | Triaged | 2019-04-04 | [FHIR-20674](./FHIR-20674.md) · [Jira](https://jira.hl7.org/browse/FHIR-20674) |
| FHIR-25040 | VhDir Search Parameters involving extensions are incorrect | Triaged | 2019-10-22 | [FHIR-25040](./FHIR-25040.md) · [Jira](https://jira.hl7.org/browse/FHIR-25040) |
| FHIR-43338 | Discuss Identifier Status w/ FHIR-I | Triaged | 2023-12-04 | [FHIR-43338](./FHIR-43338.md) · [Jira](https://jira.hl7.org/browse/FHIR-43338) |

---

## Validated Healthcare Directory (VhDir) (FHIR)

*1 ticket(s) targeting the **Validated Healthcare Directory (VhDir) (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-19865 | contains search parameter should be defined as an operation | Triaged | 2019-01-04 | [FHIR-19865](./FHIR-19865.md) · [Jira](https://jira.hl7.org/browse/FHIR-19865) |

---

*Generated automatically. Re-run `generate-indexes.py` to update.*
