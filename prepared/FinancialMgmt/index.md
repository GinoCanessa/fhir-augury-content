# Financial Mgmt — Prepared Tickets

This index lists all **43** prepared tickets for the **Financial Mgmt** work group, grouped by specification.

> **Prepared tickets** have been reviewed and have proposed dispositions ready for work-group consideration. They are sorted by date within each group so the oldest outstanding items are addressed first.

---

## Table of Contents

- [FHIR Core (FHIR)](#fhir-core-fhir)
- [US CARIN Blue Button (FHIR)](#us-carin-blue-button-fhir)
- [US Da Vinci ATR (FHIR)](#us-da-vinci-atr-fhir)
- [US Da Vinci PCDE (FHIR)](#us-da-vinci-pcde-fhir)
- [US Da Vinci PDex (FHIR)](#us-da-vinci-pdex-fhir)
- [US Da Vinci PDex (FHIR) | **Work Group:** Financial Mgmt](#us-da-vinci-pdex-fhir-work-group-financial-mgmt)
- [US Da Vinci Patient Cost Transparency (PCT) (FHIR)](#us-da-vinci-patient-cost-transparency-pct-fhir)
- [US Electronic Long-Term Services & Supports (eLTSS) (FHIR)](#us-electronic-long-term-services-supports-eltss-fhir)
- [Unspecified](#unspecified)

---

## FHIR Core (FHIR)

*26 ticket(s) targeting the **FHIR Core (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-13054 | Clarification of ValueSet: Contract Term Subtype Codes | Triaged | 2017-03-15 | [FHIR-13054](./FHIR-13054.md) · [Jira](https://jira.hl7.org/browse/FHIR-13054) |
| FHIR-20152 | Contract legal state machine doesn't match contract-legalstate value set | Triaged | 2019-01-16 | [FHIR-20152](./FHIR-20152.md) · [Jira](https://jira.hl7.org/browse/FHIR-20152) |
| FHIR-20154 | Incomplete definition of Contract.contentDerivative as legally executed contract | Triaged | 2019-01-16 | [FHIR-20154](./FHIR-20154.md) · [Jira](https://jira.hl7.org/browse/FHIR-20154) |
| FHIR-20160 | Contract.term.offer.topic requirements extra text in reference to valuedItem | Triaged | 2019-01-16 | [FHIR-20160](./FHIR-20160.md) · [Jira](https://jira.hl7.org/browse/FHIR-20160) |
| FHIR-20344 | Contract.signer should just be the Signature datatype | Triaged | 2019-02-04 | [FHIR-20344](./FHIR-20344.md) · [Jira](https://jira.hl7.org/browse/FHIR-20344) |
| FHIR-20792 | Contract has duplicated other resources codesystem and valueset | Triaged | **Priority:** Medium | **Type:** Change Request | 2019-04-10 | **Updated:** 2025-02-24 | [FHIR-20792](./FHIR-20792.md) · [Jira](https://jira.hl7.org/browse/FHIR-20792) |
| FHIR-22142 | Contract.term.action.performer - target list needs fixing | Triaged | 2019-05-13 | [FHIR-22142](./FHIR-22142.md) · [Jira](https://jira.hl7.org/browse/FHIR-22142) |
| FHIR-22704 | Why doesn't the Contract Resource state model map to the common state model for all FHIR resources | Triaged | 2019-06-15 | [FHIR-22704](./FHIR-22704.md) · [Jira](https://jira.hl7.org/browse/FHIR-22704) |
| FHIR-24915 | Harmonize Coverage and InsurancePlan type | Triaged | 2019-10-09 | [FHIR-24915](./FHIR-24915.md) · [Jira](https://jira.hl7.org/browse/FHIR-24915) |
| FHIR-24916 | Harmonize Coverage and InsurancePlan type | Triaged | 2019-10-09 | [FHIR-24916](./FHIR-24916.md) · [Jira](https://jira.hl7.org/browse/FHIR-24916) |
| FHIR-25618 | The subject of a contract could be a provider or a provider at a certain location | Triaged | 2020-01-24 | [FHIR-25618](./FHIR-25618.md) · [Jira](https://jira.hl7.org/browse/FHIR-25618) |
| FHIR-26710 | Define NUBC data elements as backbone data elements | Triaged | 2020-04-07 | [FHIR-26710](./FHIR-26710.md) · [Jira](https://jira.hl7.org/browse/FHIR-26710) |
| FHIR-26712 | Define the Diagnosis Related Group (DRG) as a backbone element | Triaged | 2020-04-07 | [FHIR-26712](./FHIR-26712.md) · [Jira](https://jira.hl7.org/browse/FHIR-26712) |
| FHIR-26717 | Define NCPDP data elements as EOB backbone data elements | Triaged | 2020-04-07 | [FHIR-26717](./FHIR-26717.md) · [Jira](https://jira.hl7.org/browse/FHIR-26717) |
| FHIR-26720 | Define Claim Received Date as an EOB backbone element | Triaged | 2020-04-07 | [FHIR-26720](./FHIR-26720.md) · [Jira](https://jira.hl7.org/browse/FHIR-26720) |
| FHIR-26721 | Define the various payer adjudication amount fields in the EOB adjudication section | Triaged | 2020-04-07 | [FHIR-26721](./FHIR-26721.md) · [Jira](https://jira.hl7.org/browse/FHIR-26721) |
| FHIR-26722 | Define allowed units in the detail section of the EOB resource | Triaged | 2020-04-07 | [FHIR-26722](./FHIR-26722.md) · [Jira](https://jira.hl7.org/browse/FHIR-26722) |
| FHIR-26966 | Contract state machine shows complex workflow that seem to conflate different patterns | Triaged | 2020-04-27 | [FHIR-26966](./FHIR-26966.md) · [Jira](https://jira.hl7.org/browse/FHIR-26966) |
| FHIR-29766 | Changes to support regulated authorizations | Triaged | 2020-11-20 | [FHIR-29766](./FHIR-29766.md) · [Jira](https://jira.hl7.org/browse/FHIR-29766) |
| FHIR-31035 | Add a value to related claim relationship code system | Triaged | 2021-02-09 | [FHIR-31035](./FHIR-31035.md) · [Jira](https://jira.hl7.org/browse/FHIR-31035) |
| FHIR-31039 | Replace values in Procedure Type Code System | Triaged | 2021-02-10 | [FHIR-31039](./FHIR-31039.md) · [Jira](https://jira.hl7.org/browse/FHIR-31039) |
| FHIR-53930 | ClaimResponse is Challenging to Balance / Adjudication Options Too Flexible | Triaged | 2026-01-09 | [FHIR-53930](./FHIR-53930.md) · [Jira](https://jira.hl7.org/browse/FHIR-53930) |
| FHIR-53935 | ClaimResponse Should Link to Original ClaimResponse When Sent as Reversal or Re-adjudication | Triaged | 2026-01-09 | [FHIR-53935](./FHIR-53935.md) · [Jira](https://jira.hl7.org/browse/FHIR-53935) |
| FHIR-54005 | Standardize spelling of "payer" - FM | Triaged | 2026-01-12 | [FHIR-54005](./FHIR-54005.md) · [Jira](https://jira.hl7.org/browse/FHIR-54005) |
| FHIR-54075 | CoverageEligibilityResponse alignment with X12 | Triaged | 2026-01-13 | [FHIR-54075](./FHIR-54075.md) · [Jira](https://jira.hl7.org/browse/FHIR-54075) |
| FHIR-54444 | Financial Module does not contain all of the Resources listed in Categorized Resource List | Triaged | 2026-01-15 | [FHIR-54444](./FHIR-54444.md) · [Jira](https://jira.hl7.org/browse/FHIR-54444) |

---

## US CARIN Blue Button (FHIR)

*6 ticket(s) targeting the **US CARIN Blue Button (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-34260 | Care Team Role Code System | Triaged | 2021-11-05 | [FHIR-34260](./FHIR-34260.md) · [Jira](https://jira.hl7.org/browse/FHIR-34260) |
| FHIR-35389 | Defining URL should be anchored in THO not hl7.org/fhir.1 - CodeSystem-C4BBAdjudication | Triaged | 2022-01-10 | [FHIR-35389](./FHIR-35389.md) · [Jira](https://jira.hl7.org/browse/FHIR-35389) |
| FHIR-35390 | Defining URL should be anchored in THO not hl7.org/fhir.2 - CodeSystem-C4BBAdjudicationDiscriminator | Triaged | 2022-01-10 | [FHIR-35390](./FHIR-35390.md) · [Jira](https://jira.hl7.org/browse/FHIR-35390) |
| FHIR-35391 | Defining URL should be anchored in THO not hl7.org/fhir.3 - CodeSystem-C4BBClaimCareTeamRole | Triaged | 2022-01-10 | [FHIR-35391](./FHIR-35391.md) · [Jira](https://jira.hl7.org/browse/FHIR-35391) |
| FHIR-35392 | Defining URL should be anchored in THO not hl7.org/fhir.4 - CodeSystem-C4BBClaimDiagnosisType | Triaged | 2022-01-10 | [FHIR-35392](./FHIR-35392.md) · [Jira](https://jira.hl7.org/browse/FHIR-35392) |
| FHIR-48340 | Submit value sets to THO | Triaged | 2024-09-21 | [FHIR-48340](./FHIR-48340.md) · [Jira](https://jira.hl7.org/browse/FHIR-48340) |

---

## US Da Vinci ATR (FHIR)

*2 ticket(s) targeting the **US Da Vinci ATR (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-39522 | Reconcile atr-coverage and hrex-coverage profiles | Triaged | 2022-11-29 | [FHIR-39522](./FHIR-39522.md) · [Jira](https://jira.hl7.org/browse/FHIR-39522) |
| FHIR-56178 | Request new operation for Member Change | Triaged | 2026-03-25 | [FHIR-56178](./FHIR-56178.md) · [Jira](https://jira.hl7.org/browse/FHIR-56178) |

---

## US Da Vinci PCDE (FHIR)

*1 ticket(s) targeting the **US Da Vinci PCDE (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-27782 | CarePlan.goal and CarePlan.careTeam should be zero'd out | Triaged | 2020-06-05 | [FHIR-27782](./FHIR-27782.md) · [Jira](https://jira.hl7.org/browse/FHIR-27782) |

---

## US Da Vinci PDex (FHIR)

*3 ticket(s) targeting the **US Da Vinci PDex (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-51188 | Retrieving Inactive/Terminated Provider Data Records through BULK FHIR API | Triaged | 2024-04-17 | [FHIR-51188](./FHIR-51188.md) · [Jira](https://jira.hl7.org/browse/FHIR-51188) |
| FHIR-55987 | Provider access: remove requirement to store bulk export details as extensions | Triaged | 2026-03-02 | [FHIR-55987](./FHIR-55987.md) · [Jira](https://jira.hl7.org/browse/FHIR-55987) |
| FHIR-56075 | Discrepancies in between operation for $provider-member-match between IG and package operation definition | Triaged | — | [FHIR-56075](./FHIR-56075.md) · [Jira](https://jira.hl7.org/browse/FHIR-56075) |

---

## US Da Vinci PDex (FHIR) | **Work Group:** Financial Mgmt

*1 ticket(s) targeting the **US Da Vinci PDex (FHIR) | **Work Group:** Financial Mgmt** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-53616 | Add more details to the Bulk Provider Request for Data showing the Asynchronous Response Pattern | — | 2025-12-11 | **Updated:** 2026-03-25 | **Resolution:** Persuasive | [FHIR-53616](./FHIR-53616.md) · [Jira](https://jira.hl7.org/browse/FHIR-53616) |

---

## US Da Vinci Patient Cost Transparency (PCT) (FHIR)

*1 ticket(s) targeting the **US Da Vinci Patient Cost Transparency (PCT) (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-51665 | Support for Real-time synchronous response with AEOB Packet to the gfe-submit | Triaged | 2025-08-06 | [FHIR-51665](./FHIR-51665.md) · [Jira](https://jira.hl7.org/browse/FHIR-51665) |

---

## US Electronic Long-Term Services & Supports (eLTSS) (FHIR)

*1 ticket(s) targeting the **US Electronic Long-Term Services & Supports (eLTSS) (FHIR)** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-23045 | Beneficiary Demographics - Person Identifier Type - Coding Medicaid ID | Triaged | 2019-08-02 | [FHIR-23045](./FHIR-23045.md) · [Jira](https://jira.hl7.org/browse/FHIR-23045) |

---

## Unspecified

*2 ticket(s) targeting the **Unspecified** specification.*

| Key | Title | Status | Date | Links |
|-----|-------|--------|------|-------|
| FHIR-13053 | Clarifications on Value Set Contract Signer Type Codes | Triaged | **Priority:** Medium | **Type:** Change Request | 2017-03-15 | **Updated:** 2025-02-24 | [FHIR-13053](./FHIR-13053.md) · [Jira](https://jira.hl7.org/browse/FHIR-13053) |
| FHIR-26719 | Define Provider Network Contracting Status in the adjudication section of the EOB Resource | Triaged | 2020-04-07 | [FHIR-26719](./FHIR-26719.md) · [Jira](https://jira.hl7.org/browse/FHIR-26719) |

---

*Generated automatically. Re-run `generate-indexes.py` to update.*
