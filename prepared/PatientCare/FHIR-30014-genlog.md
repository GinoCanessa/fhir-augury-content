# Session Log: FHIR-30014

**Phase:** prepared
**Work Group:** PatientCare
**Source File:** `prepared/PatientCare/FHIR-30014.md`

---

## Session Information

| Field | Value |
|-------|-------|
| **Session ID** | `f87a5def-b7e0-4b3b-ae40-d0467cd19b74` |
| **Session Summary** | Continue Preparing Tickets |
| **Agent ID** | `prep-FHIR-30014` |
| **Agent Type** | general-purpose |
| **Model** | claude-opus-4.6-1m |
| **Task Started** | 2026-04-10 00:54:48 UTC |
| **Duration** | 99s |
| **Total Tool Calls** | 14 |
| **Total Tokens** | 265,842 |

---

## Task Prompt

````markdown
You are preparing a FHIR Jira ticket for workgroup review.

## Ticket: FHIR-30014
## Work Group: Patient Care
## Output File: C:\ai\git\fhir-augury-content\prepared\PatientCare\FHIR-30014.md

## Data Access
Use FhirAugury MCP tools (preferred).

## Step 1: Gather Ticket and Cross-References (run in parallel)
- FhirAugury-get_item(source="jira", id="FHIR-30014", includeComments=true, includeContent=true, includeSnapshot=true)
- FhirAugury-cross_referenced(value="FHIR-30014", limit=50)

## Step 2-4: Fetch related Jira tickets, Zulip conversations (also search: FhirAugury-content_search(values="FHIR-30014", sources="zulip", limit=10)), and note GitHub items.

## Step 5: Build the Report using the standard ticket review structure.

## Rules - Use ONLY data from MCP/CLI. Do not fabricate. Be specific in dispositions. The recommendation must pick one.

## Final Step
Save to: C:\ai\git\fhir-augury-content\prepared\PatientCare\FHIR-30014.md
````

---

## Agent Execution Log

### Turn 1

**Assistant:**

Let me fetch the most relevant related tickets in parallel.

<details>
<summary>`FhirAugury-get_item`</summary>

**Result:**
```
# [jira] FHIR-30014: Merge the Functional Status IG and Cognitive Status IG

- **status:** Triaged
- **type:** Change Request
- **priority:** Highest
- **work_group:** Patient Care
- **specification:** US PACIO Functional Status (FHIR)
- **resolution:** Unresolved
- **assignee:** Unassigned
- **reporter:** Daniel Vreeman
- **Created:** 2020-12-16T00:32:14+00:00
- **Updated:** 2021-11-04T15:00:18+00:00
- **URL:** https://jira.hl7.org/browse/FHIR-30014

## Content
<p>After reviewing both PACIO IGs (Functional Status and Cognitive Status), it is unclear why two separate IGs are needed. There is substantial overlap between the two implementation guides, and no discernable differences in the driving use cases. To the reader, it feels as though they are &gt; 80% of the narrative is the same. Although a few parts (e.g. extensions) differ, the detail of the profiles and element bindings are basically the same. Splitting them into two IGs and having different profile names for subsections of content drawn from the same overall instrument (e.g. the CMS-required patient assessment instruments like MDS, OASIS, etc) imposes additional burden on implementers (and receivers). The rationale for splitting the IGs was not described in the IG. We propose merging the two IGs into a common core for functioning data, with the small number of domain-specific profiles that are needed being organized into subsections. Looking ahead to the other kinds of content being contemplated by the PACIO project, we believe this will be a more robust foundation.</p>

<p>Is there value in having two IGs? Please stongly consider merging the two IGs.</p>

## Comments (3)

### lynn_laakso — 2021-11-04T15:00:18+00:00
<p>re-opening for consideration post STU1 publication</p>

### lynn_laakso — 2021-11-04T15:00:18+00:00
<p>Reverted previous resolution: Considered for Future Use made 2021-06-28 00:00:00.0 with vote Caleb Wan / Jay Lyle: 6 - 0 - 0//(Impact: null; Category: null; Version: null)//The PACIO Project is working the Gravity Project to address the issue where the only difference between related profiles on two different use cases is the code system/value set.  Right now, both are referencing LOINC generally, but that may change to use a more focused set of LOINC codes in the future, which would mean that Functional Status and Cognitive Status would use different value sets.  A mechanism is being investigated for being able to determine the appropriate code system/value set at run time from a single profile.  This effort is just getting started, however, and will take time to develop and build consensus.</p>

### jiraadmin — 2020-12-16T00:32:16+00:00
<p>Imported from spreadsheet by jduteau - comment #21</p>


```

</details>

<details>
<summary>`FhirAugury-get_item`</summary>

**Result:**
```
# [jira] FHIR-49422: Update PACIO IG references

- **status:** Resolved - No Change
- **type:** Technical Correction
- **priority:** Highest
- **specification:** Personal Health Record (FHIR)
- **resolution:** Retracted
- **assignee:** Unassigned
- **reporter:** LizTuri
- **Created:** 2025-01-17T21:32:02+00:00
- **Updated:** 2025-01-17T21:39:11+00:00
- **URL:** https://jira.hl7.org/browse/FHIR-49422

## Content
<p>Submitted on behalf of Matt Elrod:</p>

<p> </p>

<p>Existing Text:</p>

<p>PACIO - Cognitive Status</p>

<p>PACIO - Functional Status</p>

<p> </p>

<p>New Text:</p>

<p>PACIO - Personal Functioning and Engagement (<a href="https://build.fhir.org/ig/HL7/fhir-pacio-pfe/)" class="external-link" target="_blank" rel="nofollow noopener">https://build.fhir.org/ig/HL7/fhir-pacio-pfe/)</a></p>

<p> </p>

<p>Comment: PACIO is in the process of archiving the Functional and Cognitive Status IGs as they have ben replaced with the PACIO Personal Functioning and Engagement IG</p>

```

</details>

<details>
<summary>`FhirAugury-get_item`</summary>

**Result:**
```
# [jira] FHIR-49425: Update and review PACIO IG references

- **status:** Triaged
- **type:** Change Request
- **priority:** Highest
- **work_group:** Patient Empowerment
- **specification:** Personal Health Record (FHIR)
- **resolution:** Persuasive with Modification
- **assignee:** Unassigned
- **reporter:** LizTuri
- **Created:** 2025-01-17T21:41:14+00:00
- **Updated:** 2025-03-13T21:02:43+00:00
- **URL:** https://jira.hl7.org/browse/FHIR-49425

## Content
<p>Submitted on behalf of Matt Elrod:</p>

<p> </p>

<p>Existing Text:</p>

<p>PACIO - Cognitive Status</p>

<p>PACIO - Functional Status</p>

<p> </p>

<p>New Text:</p>

<p>PACIO - Personal Functioning and Engagement (<a href="https://build.fhir.org/ig/HL7/fhir-pacio-pfe/)" class="external-link" target="_blank" rel="nofollow noopener">https://build.fhir.org/ig/HL7/fhir-pacio-pfe/)</a></p>

<p> </p>

<p>Comment: PACIO is in the process of archiving the Functional and Cognitive Status IGs as they have ben replaced with the PACIO Personal Functioning and Engagement IG. </p>

## Comments (1)

### awatson1978 — 2025-01-30T22:40:15+00:00
<p>Will update.</p>


```

</details>

<details>
<summary>`FhirAugury-get_item`</summary>

**Result:**
```
# [jira] FHIR-31590: PACIO Cognitive Status/Functional Status profiles to base on Observation resource instead of US Core Observation Lab Profile

- **status:** Resolved - No Change
- **type:** Comment
- **priority:** Medium
- **work_group:** Cross-Group Projects
- **specification:** FHIR Core (FHIR)
- **resolution:** Considered - No action required
- **assignee:** Unassigned
- **reporter:** Caleb Wan
- **labels:** USCoreVarianceRequest
- **Created:** 2021-03-23T05:43:12+00:00
- **Updated:** 2021-08-05T16:28:59+00:00
- **URL:** https://jira.hl7.org/browse/FHIR-31590

## Content
<p>The PACIO Cognitive Status and Functional Status IGs are intended to be used for the exchange of observation data collected through the use of assessment tools, instruments, screens, or other types of structured resources in various healthcare settings and home-and community-based services. Therefore, we believe the PACIO IG profiles should be based on the Observation resource instead of the US Core Observation Lab profile which mainly concerns with lab results and requires a category code of ‘laboratory’.</p>

<p> </p>

<p>We request an exception to the requirement to use a US Core Profile on Observation for this specific use case.</p>

## Comments (1)

### feisenberg — 2021-03-25T17:10:56+00:00
<p>Motion to approve variance for non-laboratory, non-vital sign resource:<br/>
Eric Haas/Brett Marquard: 14-0-1<br/>
Cross-Group Project call March 25, 2021.</p>

<p>As a discussion point, CGP notes that observations other than laboratory and vital signs are increasingly needed. Hans and Craig suggest a FHIR tracker for US Core.</p>


```

</details>

<details>
<summary>`FhirAugury-get_item`</summary>

**Result:**
```
# [jira] FHIR-29978: Clarify IG's scope by expanding the narrative

- **status:** Triaged
- **type:** Change Request
- **priority:** Medium
- **work_group:** Patient Care
- **specification:** US PACIO Cognitive Status (FHIR)
- **resolution:** Unresolved
- **assignee:** Unassigned
- **reporter:** Daniel Vreeman
- **Created:** 2020-12-15T04:04:59+00:00
- **Updated:** 2021-11-04T14:58:59+00:00
- **URL:** https://jira.hl7.org/browse/FHIR-29978

## Content
<p>The narrative does not truly describe the scope of this IG. Please help readers and implementers by addressing the central issue: how does this IG define "cognitive status" and what kinds of information is it intended to cover.</p>

<p>Let's unpack this issue a little further. </p>

<p>What anchoring framework or standard terminology is used as the basis for defining the domain?  Should it include assessments related to intellectual disabilities, learning disorders, or substance-induced impairments? What about the range of neuropsychological functional abilities/limitations? We recommend that both PACIO IGs anchor their labels and concept domains to the most well-established international framework for describing functioning and disability: the WHO's International Classification of Functioning, Disability and Health (ICF).  The ICF framework will also be part of future ICD-11 which increases the validity of the framework and facilitates a stucture that can be used globally. Such a framework will be invaluable when trying to represent content from complex domains.</p>

<p>Further, "status" implies a singular axis of assessment. From both the ICF and the CMS-required assessment instruments, it is clear that several dimensions of functioning can be described (current performance, capability, goals, etc) and are related to both physiological, personal, and environmental factors. "Status" does not adequately capture this breadth. Rather, we propose (in conjunction with our recommendation to merge the IGs) that a simpler umbrella term of "functioning" be applied and that subdomain areas be identified as needed using the ICF's conceptual framework. For example, mental functioning. This is consistent with the current proposals in the USCDI.</p>

<p>Last, the scope of both IGs should be clarified with respect to the kinds of data (e.g. Resource types) that have been modeled. Rightly so, there has been heavy focus on Observations. Is it intended to only cover content contained in the CMS-required assessment instruments? Further, it is not clear whether representation of Conditions (diagnoses), Goals, Medications, Interventions, etc are in scope (now or in the future).</p>

## Comments (3)

### lynn_laakso — 2021-11-04T14:58:59+00:00
<p>re-opening for consideration post STU1 publication</p>

### lynn_laakso — 2021-11-04T14:58:59+00:00
<p>Reverted previous resolution: Considered for Future Use made 2021-06-28 00:00:00.0 with vote Caleb Wan / Jay Lyle: 6 - 0 - 0//(Impact: null; Category: null; Version: null)//* Other ballot comments also raised this issue of the need of a definition for cognitive status. In addressing those ballot comments, PACIO has come up with a definition and also added statements to emphasize that the IG is intended to include more broadly clinical domains in mental functioning observations. PACIO is using the conceptual framework of the International Classification of Function (ICF) and will in the future to expand the IG to cover information such as memory functions, orientation functions, attention functions, mood, etc.</p>
<ul>
	<li>Another issue the ballot comment raised concerns the IG title “Cognitive Status.” It was pointed out that the term “status” used in the title implies a singular axis of assessment and does not adequately capture several dimensions of functioning. Will set aside this issue for future consideration and will address it along with <a href="https://jira.hl7.org/browse/FHIR-29977" title="Merge the Functional Status IG 

... *(truncated, 4279 chars total)*
```

</details>

### Turn 2

<details>
<summary>`FhirAugury-get_item`</summary>

**Arguments:**
```json
{
  "source": "jira",
  "id": "FHIR-30014",
  "includeComments": true,
  "includeContent": true,
  "includeSnapshot": true
}
```

**Result:**
```
# [jira] FHIR-30014: Merge the Functional Status IG and Cognitive Status IG

- **status:** Triaged
- **type:** Change Request
- **priority:** Highest
- **work_group:** Patient Care
- **specification:** US PACIO Functional Status (FHIR)
- **resolution:** Unresolved
- **assignee:** Unassigned
- **reporter:** Daniel Vreeman
- **Created:** 2020-12-16T00:32:14+00:00
- **Updated:** 2021-11-04T15:00:18+00:00
- **URL:** https://jira.hl7.org/browse/FHIR-30014

## Content
<p>After reviewing both PACIO IGs (Functional Status and Cognitive Status), it is unclear why two separate IGs are needed. There is substantial overlap between the two implementation guides, and no discernable differences in the driving use cases. To the reader, it feels as though they are &gt; 80% of the narrative is the same. Although a few parts (e.g. extensions) differ, the detail of the profiles and element bindings are basically the same. Splitting them into two IGs and having different profile names for subsections of content drawn from the same overall instrument (e.g. the CMS-required patient assessment instruments like MDS, OASIS, etc) imposes additional burden on implementers (and receivers). The rationale for splitting the IGs was not described in the IG. We propose merging the two IGs into a common core for functioning data, with the small number of domain-specific profiles that are needed being organized into subsections. Looking ahead to the other kinds of content being contemplated by the PACIO project, we believe this will be a more robust foundation.</p>

<p>Is there value in having two IGs? Please stongly consider merging the two IGs.</p>

## Comments (3)

### lynn_laakso — 2021-11-04T15:00:18+00:00
<p>re-opening for consideration post STU1 publication</p>

### lynn_laakso — 2021-11-04T15:00:18+00:00
<p>Reverted previous resolution: Considered for Future Use made 2021-06-28 00:00:00.0 with vote Caleb Wan / Jay Lyle: 6 - 0 - 0//(Impact: null; Category: null; Version: null)//The PACIO Project is working the Gravity Project to address the issue where the only difference between related profiles on two different use cases is the code system/value set.  Right now, both are referencing LOINC generally, but that may change to use a more focused set of LOINC codes in the future, which would mean that Functional Status and Cognitive Status would use different value sets.  A mechanism is being investigated for being able to determine the appropriate code system/value set at run time from a single profile.  This effort is just getting started, however, and will take time to develop and build consensus.</p>

### jiraadmin — 2020-12-16T00:32:16+00:00
<p>Imported from spreadsheet by jduteau - comment #21</p>


## Snapshot
# FHIR-30014: Merge the Functional Status IG and Cognitive Status IG

**Status:** Triaged  
**Type:** Change Request  
**Priority:** Highest  
**Resolution:** Unresolved  
**Assignee:** Unassigned  
**Reporter:** Daniel Vreeman  
**Work Group:** Patient Care  
**Specification:** US PACIO Functional Status (FHIR)  
**Created:** 2020-12-16  
**Updated:** 2021-11-04  

## Description

<p>After reviewing both PACIO IGs (Functional Status and Cognitive Status), it is unclear why two separate IGs are needed. There is substantial overlap between the two implementation guides, and no discernable differences in the driving use cases. To the reader, it feels as though they are &gt; 80% of the narrative is the same. Although a few parts (e.g. extensions) differ, the detail of the profiles and element bindings are basically the same. Splitting them into two IGs and having different profile names for subsections of content drawn from the same overall instrument (e.g. the CMS-required patient assessment instruments like MDS, OASIS, etc) imposes additional burden on implementers (and receivers). The rationale for splitting the IGs was not described in the IG. We propose merging the two IGs into a common core for func

... *(truncated, 5390 chars total)*
```

</details>

<details>
<summary>`FhirAugury-cross_referenced`</summary>

**Result:**
```
No cross-referenced references found for "FHIR-30014".
```

</details>

<details>
<summary>`FhirAugury-content_search`</summary>

**Result:**
```
No results found.
```

</details>

### Turn 3

<details>
<summary>`create` — create a new file at C:\ai\git\fhir-augury-content\prepared\PatientCare\FHIR-30014.md.</summary>

**Arguments:**
```json
{
  "path": "C:\\ai\\git\\fhir-augury-content\\prepared\\PatientCare\\FHIR-30014.md",
  "file_text": "# FHIR-30014: Merge the Functional Status IG and Cognitive Status IG\n\n## Summary\n\n| Field | Value |\n|-------|-------|\n| **Ticket** | [FHIR-30014](https://jira.hl7.org/browse/FHIR-30014) |\n| **Status** | Triaged |\n| **Type** | Change Request |\n| **Priority** | Highest |\n| **Resolution** | Unresolved |\n| **Work Group** | Patient Care |\n| **Specification** | US PACIO Functional Status (FHIR) |\n| **Reporter** | Daniel Vreeman |\n| **Assignee** | Unassigned |\n| **Created** | 2020-12-16 |\n| **Updated** | 2021-11-04 |\n\n## Description\n\nDaniel Vreeman proposes merging the PACIO Functional Status IG and Cognitive Status IG into a single combined IG. The rationale:\n\n- **>80% narrative overlap** between the two IGs with no discernable differences in driving use cases.\n- Profile structures and element bindings are essentially the same; only a few parts (e.g., extensions) differ.\n- Splitting into two IGs with different profile names for content drawn from the same instruments (e.g., CMS-required MDS, OASIS) **imposes unnecessary burden** on implementers and receivers.\n- The rationale for the split was not described in either IG.\n- A merged IG with a common core and domain-specific subsections would provide a **more robust foundation** for future PACIO project work.\n\n## Keywords\n\nPACIO, Functional Status, Cognitive Status, merge, implementation guide, CMS, MDS, OASIS, assessment instruments, profile overlap, implementer burden, Personal Functioning and Engagement\n\n## Comment History\n\n| Date | Author | Summary |\n|------|--------|---------|\n| 2020-12-16 | jiraadmin | Imported from spreadsheet by jduteau (comment #21) |\n| 2021-11-04 | lynn_laakso | Reverted previous resolution of \"Considered for Future Use\" (voted 6-0-0, Caleb Wan / Jay Lyle, 2021-06-28). Original rationale: PACIO was working with Gravity Project on differentiating code systems/value sets between functional and cognitive status, with a mechanism to determine the appropriate value set at runtime from a single profile. |\n| 2021-11-04 | lynn_laakso | Re-opened for consideration post STU1 publication |\n\n## Related Jira Tickets\n\n| Ticket | Title | Status | Resolution | Relevance |\n|--------|-------|--------|------------|-----------|\n| [FHIR-29977](https://jira.hl7.org/browse/FHIR-29977) | Merge the Functional Status IG and Cognitive Status IG. | Triaged | Unresolved | **Near-duplicate** \u2014 identical text, same reporter, filed against US PACIO Cognitive Status IG. Same comment history (voted Considered for Future Use 6-0-0, then re-opened). |\n| [FHIR-49422](https://jira.hl7.org/browse/FHIR-49422) | Update PACIO IG references | Resolved - No Change | Retracted | Confirms that PACIO is **archiving** the Functional and Cognitive Status IGs, replaced by the PACIO Personal Functioning and Engagement (PFE) IG. |\n| [FHIR-49425](https://jira.hl7.org/browse/FHIR

... *(truncated, 8699 chars total)*
```

**Result:**
```
Created file C:\ai\git\fhir-augury-content\prepared\PatientCare\FHIR-30014.md with 8537 characters
```

</details>

### Turn 4

**Assistant:**

Now let me fetch the related ticket FHIR-30014 mentioned in comments, and search for broader Zulip discussions on the topic.

<details>
<summary>`FhirAugury-get_item`</summary>

**Result:**
```
# [jira] FHIR-30014: Merge the Functional Status IG and Cognitive Status IG

- **status:** Triaged
- **type:** Change Request
- **priority:** Highest
- **work_group:** Patient Care
- **specification:** US PACIO Functional Status (FHIR)
- **resolution:** Unresolved
- **assignee:** Unassigned
- **reporter:** Daniel Vreeman
- **Created:** 2020-12-16T00:32:14+00:00
- **Updated:** 2021-11-04T15:00:18+00:00
- **URL:** https://jira.hl7.org/browse/FHIR-30014

## Content
<p>After reviewing both PACIO IGs (Functional Status and Cognitive Status), it is unclear why two separate IGs are needed. There is substantial overlap between the two implementation guides, and no discernable differences in the driving use cases. To the reader, it feels as though they are &gt; 80% of the narrative is the same. Although a few parts (e.g. extensions) differ, the detail of the profiles and element bindings are basically the same. Splitting them into two IGs and having different profile names for subsections of content drawn from the same overall instrument (e.g. the CMS-required patient assessment instruments like MDS, OASIS, etc) imposes additional burden on implementers (and receivers). The rationale for splitting the IGs was not described in the IG. We propose merging the two IGs into a common core for functioning data, with the small number of domain-specific profiles that are needed being organized into subsections. Looking ahead to the other kinds of content being contemplated by the PACIO project, we believe this will be a more robust foundation.</p>

<p>Is there value in having two IGs? Please stongly consider merging the two IGs.</p>

## Comments (3)

### lynn_laakso — 2021-11-04T15:00:18+00:00
<p>re-opening for consideration post STU1 publication</p>

### lynn_laakso — 2021-11-04T15:00:18+00:00
<p>Reverted previous resolution: Considered for Future Use made 2021-06-28 00:00:00.0 with vote Caleb Wan / Jay Lyle: 6 - 0 - 0//(Impact: null; Category: null; Version: null)//The PACIO Project is working the Gravity Project to address the issue where the only difference between related profiles on two different use cases is the code system/value set.  Right now, both are referencing LOINC generally, but that may change to use a more focused set of LOINC codes in the future, which would mean that Functional Status and Cognitive Status would use different value sets.  A mechanism is being investigated for being able to determine the appropriate code system/value set at run time from a single profile.  This effort is just getting started, however, and will take time to develop and build consensus.</p>

### jiraadmin — 2020-12-16T00:32:16+00:00
<p>Imported from spreadsheet by jduteau - comment #21</p>


```

</details>

<details>
<summary>`FhirAugury-content_search`</summary>

**Result:**
```
No results found.
```

</details>

<details>
<summary>`FhirAugury-content_search`</summary>

**Result:**
```
No results found.
```

</details>

<details>
<summary>`FhirAugury-related_by_keyword`</summary>

**Result:**
```
## Items Related to jira:FHIR-30015 (by keyword similarity)

| Score | Source | ID | Title | Shared Keywords |
|------:|--------|-------|-------|-----------------|
| 205.085 | jira | FHIR-29978 | Clarify IG's scope by expanding the narrative | icf, framework, function, assessment, disability, instrument, neuropsychological, domain, stucture, invaluable, rightly, unpack, subdomain, scope, igs, umbrella, international, breadth, narrative, cms, goal, physiological, axis, dimension/dimensions, anchor, kind, ability, expand, environmental, heavy, cognitive, conjunction, validity, adequately, globally, singular, conceptual, cover, pacio, clarify, status, central, icd, intended, describe, facilitate, factor, performance |
| 73.922 | jira | FHIR-29978:20024 | FHIR-29978:20024 | icf, framework, function, assessment, domain, international, axis, dimension/dimensions, expand, cognitive, adequately, singular, conceptual, cover, pacio, status, intended |
| 73.307 | jira | FHIR-30015:20027 | FHIR-30015:20027 | icf, framework, function, assessment, disability, domain, international, axis, dimension/dimensions, expand, cognitive, adequately, singular, conceptual, pacio, status, intended |
| 51.885 | jira | FHIR-30014 | Merge the Functional Status IG and Cognitive Status IG | function, assessment, instrument, domain, igs, narrative, cms, kind, cognitive, pacio, status |
| 51.521 | jira | FHIR-29977 | Merge the Functional Status IG and Cognitive Status IG. | function, assessment, instrument, domain, igs, narrative, cms, kind, cognitive, pacio, status |
| 47.630 | jira | FHIR-30036 | Decription of what functional status is will provide clarity. | icf, framework, function, assessment, domain, international, cms, conceptual, pacio, status, describe, factor |
| 46.085 | jira | FHIR-29981 | Explain and justify the use of extensions for AssistanceRequired and DevicePatientUsed | icf, framework, function, assessment, instrument, cms, cognitive, clarify, status |
| 42.536 | jira | FHIR-30020 | Clarify | icf, framework, function, assessment, instrument, cms, clarify, status |
| 42.487 | jira | FHIR-38054 | Reference to ICF should provide more detailed mapping | icf, function, assessment, instrument, domain, cognitive, pacio, status |
| 37.319 | jira | FHIR-38618 | Question use of Personal and Engagement in the IG Title | icf, function, scope, igs, cover, pacio, clarify, intended, describe |

⚠️ Related-by-keyword failed for source 'GitHub': Response status code does not indicate success: 404 (Not Found).
⚠️ Related-by-keyword failed for source 'Zulip': Response status code does not indicate success: 404 (Not Found).

```

</details>

